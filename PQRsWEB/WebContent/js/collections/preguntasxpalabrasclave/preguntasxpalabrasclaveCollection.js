...
], function($, _,Backbone, preguntasxpalabrasclaveModel){//se extienden todos los métodos y atributos de la clase Backbone.Model en la coleccion actual 

var preguntasxpalabrasclaveCollection = Backbone.Collection.extend({// valores predeterminados de la coleccion actual
model: preguntasxpalabrasclaveModel, 
//Parámetros que necesita el servidor 
        url: "http://localhost:9080/PQRsWEB/webresources/edu.poli.proyecto.test.tbpreguntasxpalabrasclave",//direccion url de cada modelo y coleccion
        sync: function (method, model, options) { //Operaciones de sincronización manejadas a traves del servidor y operaciones de los modelos
            ...
            var result = Backbone.sync(method, model, _.extend(options, errorHandler));//guarda en una variable
            return result;
        }
    });
